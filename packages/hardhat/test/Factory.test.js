...
contract("Factory", (accounts) => {
  ...
  beforeEach(async function () {
    ...
    app = await Factory.new(sf.host.address, sf.agreements.cfa.address, daix.address);
  ...
  // const initTx = await app.methods
  ...
  // )
  async function createFlow(name, symbol, flowrate) {
    const initTx = await app.createFlow(name, symbol, flowrate);
  ...
  describe("Testing the Factory Contract", async function () {
    it("Case #1 - Alice receives cashflow", async () => {
      const { alice, admin } = u;
      const name = "TVLPayout";
      const symbol = "TVL";
      const flowrate = toWad(1);
      ...
      await appStatus();
      await logUsers();
      await app.addContributor(alice.address, 0);
      ...
      assert.equal(
        (await app.getContributorFlows(alice.address)).length, 1,
        "Alice has more or less than one flow"
      );
      assert.equal(
        (await app.getContributorFlows(alice.address))[0].getFlowRate(alice.address), 
        toWad(1),
        "The flow rate of Alice is not correct"
      );
    });
    ...
    //   assert.equal(initialUserData, "HODL BTC", "user data value incorrect");
    //   assert.equal(
    ...
    //   await logUsers();
    //   await alice.flow({
    //     flowRate: toWad(0.00002).toString(),
    //     recipient: u.app,
    //     userData: web3.eth.abi.encodeParameter("string", "HODL ETH"),
    //   });
    //   assert.equal(
    //     ((await alice.details()).cfa.netFlow * -1).toString(),
    //     (await u.admin.details()).cfa.netFlow.toString(),
    ...
    //   assert.equal(updatedUserData, "HODL ETH", "user data value incorrect");
    ...
    //   assert.equal(finalUserData, "", "user data value incorrect");
    //   assert.equal(
    //     ((await alice.details()).cfa.netFlow * -1).toString(),
    //     (await u.admin.details()).cfa.netFlow.toString(),
    //     "netflows aren't equal"
    ...
    //     flowRate: "1000000",
    //     recipient: u.app,
    //     userData: web3.eth.abi.encodeParameter("string", "HODL DOGE"),
    //   });
    //   assert.equal(
    //     (await u.app.details()).cfa.netFlow,
    //     0,
    ...
    //     flowRate: toWad(0.00002).toString(),
    //     recipient: u.app,
    //     userData: web3.eth.abi.encodeParameter("string", "HODL DOT"),
    //   });
    //   await bob.flow({
    //     flowRate: toWad(0.0000115).toString(),
    //     recipient: u.app,
    //     userData: web3.eth.abi.encodeParameter("string", "HODL PEOPLE"),
    //   });
    //   console.log("go forward in time");
    //   await traveler.advanceTimeAndBlock(TEST_TRAVEL_TIME);
    //   assert.equal(
  ...
  //         flowRate: "1000000",
  //         recipient: u.app,
  //         userData: web3.eth.abi.encodeParameter("string", "hello world"),
  //       });
  //       await bob.flow({
  //         flowRate: "10000000",
  //         recipient: u.app,
  //         userData: web3.eth.abi.encodeParameter("string", "good bye world"),
  //       });
  //       await carol.flow({
  //         flowRate: "10000000",
  //         recipient: u.app,
  //         userData: web3.eth.abi.encodeParameter("string", "GM"),
  //       });
  //       assert.equal(
  //         (await u.app.details()).cfa.netFlow,
  //         0,
  ...
  //         flowRate: "1500000",
  //         recipient: u.app,
  //         userData: web3.eth.abi.encodeParameter("string", "GA"),
  //       });
  //       await carol.flow({
  //         flowRate: "1500000",
  //         recipient: u.app,
  //         userData: web3.eth.abi.encodeParameter("string", "GN"),
  //       });
  //       await dan.flow({
  //         flowRate: "1500000",
  //         recipient: u.app,
  //         userData: web3.eth.abi.encodeParameter("string", "WAGMI"),
  //       });
  //       assert.equal(
  //         (await u.app.details()).cfa.netFlow,
  //         0,
  ...
  //           ((await u.dan.details()).cfa.flows.outFlows[0].flowRate || 0)
  //         ).toString(),
